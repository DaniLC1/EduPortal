                            ...
                            <ul>
                                ...
                                <?php if (!empty($assignments)): ?>
                                    <?php foreach ($assignments as $a): ?>
                                        ...
                                        // Ha még nincs beadás, score = null
                                        $latest_score = $student_attempts[0]['score'] ?? null;
                                        $max_score = isset($a['max_score']) ? (int)$a['max_score'] : 0; // fallback max pontszám
                                        ?>

                                        <li>
                                            <details <?= $attempt_count > 1 ? '' : 'open' ?>>
                                                <summary>
                                                    <strong><?= htmlspecialchars($a['title']) ?></strong> –
                                                    <?= date('Y.m.d', strtotime($a['due_date'])) ?>
                                                    <br>
                                                    <?= mb_strimwidth(htmlspecialchars($a['description']), 0, 100, '...') ?>
                                                    <br>
                                                    <small>
                                                        Próbálkozás: <?= $attempt_count ?> / <?= $max_attempts ?> |
                                                        Eredmény:
                                                        <?= $latest_score !== null
                                                            ? $latest_score . " / " . $max_score . " pont"
                                                            : "– / " . $max_score . " pont" ?>
                                                    </small>
                                                </summary>
                                                ...
                                                <?php if ($attempt_count < $max_attempts || $max_attempts === '∞'): ?>
                                                    <form method="GET" action="assignment.php" style="display:inline;">
                                                        <input type="hidden" name="assignment_id" value="<?= $assignment_id ?>">
                                                        <button type="submit" class="fill-btn">✍️ Új kitöltés</button>
                                                    </form>
                                                <?php else: ?>
                                                    <p><em>Maximális próbálkozások száma elérve.</em></p>
                                                <?php endif; ?>
                                            </details>
                                        </li>
                                    <?php endforeach; ?>
